...
Эта ветка продолжает разработку AyuGram на базе Telegram Desktop 7.2.5,
сохраняя название, функции и авторство исходного проекта. Это не официальный
релиз от разработчиков upstream.

Обновление основано на [PR #460](https://github.com/AyuGram/AyuGramDesktop/pull/460).
Сборка для macOS Apple Silicon проверена в повседневной работе. Intel macOS,
Windows и Linux ещё требуют проверки этой ветки. Порядок сборки и условия
публичного выпуска описаны в [руководстве](docs/releasing.md).

Пакеты и команды пакетных менеджеров ниже относятся к upstream или сторонним
сопровождающим, а не устанавливают эту ветку. Артефакты macOS CI предназначены
для проверки и не являются подписанным публичным релизом.
